...
module.exports.connect = function (cb) {
  var host = settings.irc.host;
  var port = settings.irc.port;

  logger.info('IRC connecting to %s:%d', host, port);
  client = new irc.Client(host, settings.nick, settings.irc);
  ...
  client.on('registered', function () {
    logger.info('IRC connected to %s:%d', host, port);
    cb();
  });
};
